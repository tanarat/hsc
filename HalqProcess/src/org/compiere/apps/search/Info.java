	...
	implements ListSelectionListener
{
		...
		boolean multiSelection, String whereClause)
	{
		...
		if (tableName.equals("C_BPartner"))
			info = new InfoEvalItemReason_bak (frame, modal, WindowNo,  value, !Env.getContext(Env.getCtx(),"IsSOTrx").equals("N"),
			...
			info = new InfoAssignment (frame, modal, WindowNo, value,
					multiSelection, whereClause);
		else if (tableName.equals("HSC_BP"))
			info = new InfoGeneral (frame, modal, WindowNo, value, 
					tableName, keyColumn, 
					true, whereClause);
		else if (tableName.equals("HSC_Q_Item"))
			info = new InfoGeneral (frame, modal, WindowNo, value, 
					tableName, keyColumn, 
					true, whereClause);
		else if (tableName.equals("HSC_Eval_Item"))
			info = new InfoGeneral (frame, modal, WindowNo, value, 
					tableName, keyColumn, 
					true, whereClause);
		else if (tableName.equals("HSC_Eval_Item_Reason")){
//			info = new InfoEvalItemReason(frame, modal, WindowNo, value, tableName, keyColumn, true, whereClause);
//			System.out.println(""+Env.getCtx().getProperty("HSC_Eval_Item_ID"));
			System.out.println(WindowNo);
			System.out.println(value);
			System.out.println(tableName);
			info = new InfoGeneral (frame, modal, WindowNo, value, 
					tableName, keyColumn, 
					true, "HSC_Eval_Item_ID = "+ Env.getCtx().getProperty("HSC_Eval_Item_ID") ); //"HSC_Eval_Item_ID = "+ Env.getCtx().getProperty("2|1|HSC_Eval_Item_ID")
		}else
			info = new InfoGeneral (frame, modal, WindowNo, value, 
				tableName, keyColumn, 
				multiSelection, whereClause);
	...
	{
		Info info = new InfoEvalItemReason_bak (frame, false, WindowNo,  "",
