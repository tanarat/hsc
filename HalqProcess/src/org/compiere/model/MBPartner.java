...
public class MBPartner extends X_C_BPartner
{
	 ...
	 *	@return contacts
	 */
	public MUser[] getActiveContacts (boolean reload)
	{
		if (reload || m_contacts == null || m_contacts.length == 0)
			;
		else
	...
	public boolean hasUserRole(String roleName){
		MUser[] contacts = getContacts(true);
		
		for (int i = 0; i < contacts.length; i++) {
			MRole[] roles = contacts[i].getRoles(1000000); // Org = 'Center'
			for (int j = 0; j < roles.length; j++) {
				if(roles[j].getName().equals(roleName)) return true;
			}
		}
		return false;
	}
	...
	protected boolean afterSave (boolean newRecord, boolean success)
	{
		...
		if(getBPGroup().getName().equals("Entrepreneur") 
				&& !hasUserRole("Register User")){
			//Create user for registration
			MUser user = new MUser(this);
			user.setName(get_ID()+"");
			user.setPassword(get_ID()+"");
			user.save();
			//set user role
			MRole registerUserRole = MRole.getOf(getCtx(), "Name = 'Register User'")[0];
			MUserRoles userRoles = new MUserRoles(this.getCtx(), user.get_ID(), registerUserRole.get_ID(), this.get_TrxName());
			userRoles.save();
		}
	...
	public MBPartnerLocation getLocationByName(String locationName) {
		// TODO Auto-generated method stub
		//select * from c_bpartner_location where c_bpartner_id = 1000074 and name = 'office'
		String tableName = MBPartnerLocation.Table_Name;
		String whereClause = "IsActive = 'Y' and c_bpartner_id = ? and name = ?";
		List<MBPartnerLocation> loc =  new Query(this.p_ctx, tableName, whereClause, this.get_TrxName()).setParameters(this.get_ID(),locationName).list();
	...
	public MHSCBPStaff[] getTraining1Staff(int project_ID) {
		// TODO Auto-generated method stub
		String tableName = MHSCBPStaff.Table_Name;
		String whereClause = "IsActive = 'Y' and c_bpartner_id = ? and HSC_Halq_Project_ID = ? and IsTraining1Staff = 'Y'";
		List<MHSCBPStaff> staffs =  new Query(this.p_ctx, tableName, whereClause, this.get_TrxName()).setParameters(this.get_ID(),project_ID).list();
		return staffs.toArray(new MHSCBPStaff[staffs.size()]);
	}

	public MHSCBPStaff[] getTraining2Staff(int project_ID) {
		// TODO Auto-generated method stub
		String tableName = MHSCBPStaff.Table_Name;
		String whereClause = "IsActive = 'Y' and c_bpartner_id = ? and HSC_Halq_Project_ID = ? and IsTraining2Staff = 'Y'";
		List<MHSCBPStaff> staffs =  new Query(this.p_ctx, tableName, whereClause, this.get_TrxName()).setParameters(this.get_ID(),project_ID).list();
		return staffs.toArray(new MHSCBPStaff[staffs.size()]);
	}
